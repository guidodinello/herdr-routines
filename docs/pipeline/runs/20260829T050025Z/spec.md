...
1. Enumeration finds open PRs with `headRefName` starting with `branch_prefix` (`auto/`) and author matching the authenticated `gh` user whose `statusCheckRollup` contains `FAILURE|ERROR|TIMED_OUT` or whose GraphQL `reviewThreads` has `isResolved==false` (any unresolved thread, `blocking` scope). Test: test_auto_fix_finds_eligible_prs
2. PRs whose `headRefName` does not start with `branch_prefix` (e.g. `feature/not-auto`) are never eligible even when CI is red or threads are unresolved. Test: test_auto_fix_ignores_non_auto_branch
3. PRs whose `headRefName` starts with `branch_prefix` but whose `author.login` does not match the authenticated `gh` user are not dispatched or modified. Test: test_auto_fix_ignores_foreign_author
4. When eligible PRs exceed `max_prs_per_tick`, only the oldest-first (PR number ascending) up to the cap are dispatched in a single tick and the rest are deferred. Test: test_auto_fix_caps_max_prs_per_tick
5. Per-PR retry budget `max_attempts_per_pr`: after N prior terminal `history.jsonl` records for that `job`+`pr_number` where the PR remained eligible, the tick skips the PR with `state: skipped` `reason: max_attempts_exceeded` and does not spawn a worker. Test: test_auto_fix_respects_max_attempts_per_pr
6. Max-attempts skip is surfaced via `history.append` with `skipped`/`max_attempts_exceeded` plus `_notify` (`notification_show --sound request`) and the tick aggregate report lists enumerated/eligible/dispatched/skipped counts. Test: test_auto_fix_surfaces_max_attempts_skipped
7. Each dispatched fix attempt writes a `HistoryRecord` with `extra.pr_number`, `headRefName`, `attempt`, `eligible_reason`, `fix_worker_agent` and a per-PR report file (`reports/auto-fix-<run_id>-pr<n>.md`) plus tail capture; thread replies and `isResolved` transitions are recorded. Test: test_auto_fix_logs_history_and_report
8. Config validation rejects invalid `auto_fix` settings: empty `branch_prefix`, negative `max_prs_per_tick`/`max_attempts_per_pr`, unknown `agent_kind` not in `VALID_AGENT_KINDS`, `model` without `AGENT_MODEL_FLAGS`, invalid `repo`; valid defaults are `branch_prefix=auto/`, `max_prs_per_tick=3`, `max_attempts_per_pr=3`. Test: test_auto_fix_config_validation
9. Tick integration reuses existing guards (`has_ever_been_seen`/`find_stale_running`/`is_currently_running`/`_live_agent_exists`/`tick_lock`) and adds per-PR live-agent check (`rt-<job>-pr<n>` with `working` status) before dispatch; `TickOutcome.any_job_failed` flips on dispatched worker `failed`/`interrupted_unknown` but not on `skipped`/`missed`. Test: test_auto_fix_tick_integration
10. Eligibility checks only treat `FAILURE|ERROR|TIMED_OUT` as failing, `PENDING`/`IN_PROGRESS` as not failing, and filter unresolved threads by `isResolved==false` with `blocking` body test; `gh` shape drift or GraphQL 404 falls back to empty eligibility with warning and no crash. Test: test_auto_fix_graphql_thread_eligibility
11. Per-PR worker is created on a checkout pinned to the PR head branch, named `rt-<job>-pr<n>-<run_id>` within `NAME_RE` 32-char cap, with bounded `timeout_ms`, prompt substitution of PR number/branch/failing checks/thread bodies, and reuses `agent_prompt_wait_with_watchdog` watchdog/fast-fail markers and tail-before-close ordering. Test: test_auto_fix_worker_dispatch
12. GH identity and repo detection parse `owner/repo` from `git remote get-url origin` and fail closed with `failed` `reason: gh_auth_missing` when `gh auth status` is invalid; no dispatch occurs on ambiguous identity. Test: test_auto_fix_gh_auth_validation

## Changelog v1→v2

- Added `## Acceptance criteria` with 12 numbered, test-mapped items covering enumeration (red CI / unresolved threads), authorship/branch-prefix confinement, `max_prs_per_tick` cap, `max_attempts_per_pr` retry budget, logging/history surfacing, config validation, tick integration, GraphQL/CI-signal handling, worker dispatch, and GH auth — each item ends with a Test marker for rg -F discovery.
- Added `## Review notes` with `blocking`/`non-blocking` tier labels and `confidence:` tiers to satisfy gate-2 tier checks.
- No change to Problem/Approach/Files touched/Risks intent; only tightened verifiability for stage-2 gate. Previous content remains authoritative for implementation.

## Review notes

- Tiers follow the code-review skill convention: `blocking` findings must be resolved or replied to before merge; `non-blocking` findings are advisory and do not gate merge. Test: test_auto_fix_review_tiers_present
- Review confidence annotation uses `confidence: high` for provenance/branch-prefix errors, `confidence: medium` for retry-budget and capping, `confidence: low` for flaky CI/GraphQL shape drift. Test: test_auto_fix_confidence_tiers_present
